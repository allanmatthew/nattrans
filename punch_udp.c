...
int punch_udp(int localport, char if_name[], struct sockaddr_in *peerAddr) {
    ...
    printf("Waiting for partner data from server...\n");
    memset(buf, 0, sizeof(buf));
    ...
    int first_response = 99;
    bool returned_ping = false;
     ...
     * the first response return that data in the peer_addr */
    ...
    while(true) {
        ...
        if(this_uid == me.uid) {
            ...
            /* Return the public/private port in peerAddr */
            first_response = ((packet_t*)buf)->pkt_type;
        }
        else {
            //Send it right back
        ...
        if(returned_ping && 
           (first_response == PRIVATE_DATA || first_response == PUBLIC_DATA)){
            peerAddr->sin_family = AF_INET;
            if(first_response == PUBLIC_DATA){
                peerAddr->sin_port = partner.ip_data.public_port;
                peerAddr->sin_addr.s_addr = partner.ip_data.public_addr;
            }
            else {
                peerAddr->sin_port = partner.ip_data.private_port;
                peerAddr->sin_addr.s_addr = partner.ip_data.private_addr;
            }
            return sock_fd;
        } 
    }
